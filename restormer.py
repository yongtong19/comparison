#%% mask version of Restomer x,mask,pretrain=False
...
class Restormer(nn.Module):
    def __init__(self, num_blocks=[2, 4, 4, 6], num_heads=[1, 2, 4, 8], channels=[48, 96, 192, 384], num_refinement=4, #num_blocks=[4, 6, 6, 8]
    ...
    def forward(self, x): #, mask, pretrain):
        x0 = x
        #if pretrain:
        #  x = x * (1 - mask)
        ...
        # if not pretrain: ## only for finetuning
        #   out = self.output(fr) + x0
        # else:
        #   out = self.output(fr)
        out = self.output(fr) + x0
        return out